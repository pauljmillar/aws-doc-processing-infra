#!/usr/bin/env python3
"""
Test script for Smart Batching functionality
"""
import boto3
import time
...
def main():
    ...
    base_name = "SMARTBATCHTEST"
    document_id = f"{base_name}-{int(datetime.utcnow().timestamp())}"
    
    print(f"Testing Smart Batching with document ID: {document_id}")
    print("=" * 60)
    ...
    print("1. Uploading first file (should start processing window)...")
    ...
    print("\n2. Uploading second file (should reset window)...")
    ...
    print("\n3. Uploading third file (should reset window again)...")
    upload_to_s3(bucket_name, test_files[2], f"incoming/{test_files[2]}")
    ...
    print("\n4. Waiting for window to expire and processing to start...")
